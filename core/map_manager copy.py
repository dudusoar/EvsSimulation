...
class MapManager:
    ...
    def initialize_edge_times(self):
        ""Calculate and store traversal time for all edges""
        for u, v, data in self.projected_graph.edges(data=True):
            length = data.get('length', 0)
            data['time'] = self.calculate_edge_time(length, data)


    def get_shortest_path_nodes(self, origin: int, destination: int) -> List[int]:
        ""Get fastest path node list (based on traversal time)""
        ...
        try:
            path = nx.shortest_path(
                ...
                origin,
                destination,
                weight='time'  # Changed from 'length' to 'time'
            )
            return path if self.validate_path_time(path) else []
        
        except nx.NetworkXNoPath:
            return []
     
    def validate_path_time(self, path: List[int]) -> bool:
        ""Verify path time is realistic""
        total_time = self.calculate_route_distance(path[0], path[-1])
        return 0 < total_time < float('inf')  


    def get_shortest_path_points(self, origin: int, destination: int) -> List[Tuple[float, float]]:
        ""Get detailed coordinate points of fastest path (based on traversal time)""
        # Get path nodes
        route_nodes = self.get_shortest_path_nodes(origin, destination)
        if not route_nodes: # no path exists
            return [] 
    ...
    # Convert node path to detailed coordinate points
        ...
        for u, v in edge_nodes:
            ...
            edge_data = self.projected_graph.get_edge_data(u, v)
            if edge_data:
                # Select fastest edge (minimum time)
    ...
    def calculate_edge_time(self, length: float, edge_data: dict) -> float:
        ""More robust time calculation""
        if length <= 0:
            return float('inf')
    
        # Get speed limit with better fallback logic
        speed_kmh = self._get_speed_limit(edge_data)
        speed_ms = speed_kmh * (1000/3600)  # Convert km/h to m/s
    
        # Prevent division by zero and unrealistic speeds
        speed_ms = max(0.1, min(speed_ms, 50))  # Constrain between 0.1 m/s and 50 m/s (180 km/h)
    
        return length / speed_ms

    def _get_speed_limit(self, edge_data: dict) -> float:
        ""Extract speed limit from edge data with sensible defaults""
        default_speed = 50.0  # 50 km/h default for urban areas
    
        speed = edge_data.get('maxspeed', default_speed)
    
        if isinstance(speed, list):
            speed = speed[0]  # Take first speed if multiple exist
        ...
        for  u,v, key, data in self.projected_graph.edges(keys=True, data = True):
            ...
            if 'maxspeed' in data:
                ...
                if isinstance(speed_str, list):
                    speed_str = speed_str[0]  # Take the first value

                elif isinstance(speed_str, dict):
                    speed_str = speed_str.get('forward') or speed_str.get('backward') or ''
                    ...
                    if speed_str.isdigit():
                        ...
                        try:
                            ...
                            if 'km/h' in unit_part or 'kmh' in unit_part:
                                ...
                                speed_mps = speed_val * (1609.34 / 3600)
                            elif 'm/s' in unit_part or 'ms' in unit_part:
                                speed_mps = speed_val
                            else:
                                speed_mps = speed_val * (1000 / 3600)  # Default to km/h
                        except ValueError:
                            pass  # speed_mps remains None
    ...
    def get_shortest_path_nodes(self, origin: int, destination: int) -> List[int]:
        ...
        This version handles MultiDiGraph by collapsing it into a DiGraph
        where each edge represents the shortest travel_time between u and v.

        CORRECTION #4: Ensures correct edge is used in MultiDiGraph by aggregating minimum travel_time.
        ""

    # Step 1: Build a temporary DiGraph with minimum travel_time edges
        time_weighted_graph = nx.DiGraph()

        for u, v, data in self.projected_graph.edges(data=True):
            travel_time = data.get('travel_time', float('inf'))

        # If an edge already exists between u and v, keep the one with the shorter travel_time
            if time_weighted_graph.has_edge(u, v):
                if travel_time < time_weighted_graph[u][v]['weight']:
                    time_weighted_graph[u][v]['weight'] = travel_time
            else:
                time_weighted_graph.add_edge(u, v, weight=travel_time)

    # Step 2: Compute shortest path using the simplified DiGraph
        try:
            return nx.shortest_path(
                time_weighted_graph,
                origin,
    ...
    def get_shortest_path_points(self, origin: int, destination: int) -> List[Tuple[float, float]]:
        ...
        for u, v in edge_nodes:
            ...
            if edge_data:
                # Select shortest edge
                edge = min(edge_data.values(), key=lambda x: x.get('travel_time', float('inf')))
